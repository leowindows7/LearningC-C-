...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  if (w <= 0 || h <= 0 || numMines < 0 || numMines > w * h) {
    return NULL;
  }
  board_t * b = malloc(sizeof(*b));
  int ** ininDer = malloc(h * sizeof(ininDer));
  b->width = w;
  b->height = h;
...
int countMines(board_t * b, int x, int y) {
  ...
  for (int ny = y - 1; ny <= y + 1; ny++) {
    for (int nx = x - 1; nx <= x + 1; nx++) {
      if (ny < 0 || nx < 0 || ny >= b->height || nx >= b->width || (ny == y && nx == x)) {
        continue;
      }
      else {
        //printf("%d,%d,%d\n", ny, nx, b->board[ny][nx]);
        if (b->board[ny][nx] == HAS_MINE || b->board[ny][nx] == KNOWN_MINE) {
          count++;
        }
        // printf("%d\n", count);
      }
