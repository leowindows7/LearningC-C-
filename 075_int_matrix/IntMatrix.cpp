...
IntMatrix IntMatrix::operator+(const IntMatrix & rhs) const {
  assert((rhs.numRows == numRows) && (rhs.numColumns == numColumns));
  IntMatrix myMatrix(numRows, numColumns);
  for (int i = 0; i < numRows; i++) {
    for (int j = 0; j < numColumns; j++) {
      (*myMatrix.rows[i])[j] = (*rows[i])[j] + (*rhs.rows[i])[j];
    }
  }
  return myMatrix;
}
